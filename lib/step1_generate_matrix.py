...
#        q30cmd = """awk '/^[^@]/{FS="\t";OFS="\t";if (substr($3,1,3) == "chr") {if ($2 == 16) print $3,$4-1,$4+length($11),$1,255,"-";else print $3,$4-1,$4,$1,255,"+";}}' %s > %s"""%(conf_dict['General']['sam'],conf_dict['General']['bed'])
    ...
    wlog('generate expression matrix and individual cell qc matrix',logfile)    
    ...
    rwlog(cmd4,logfile,conf_dict['General']['dryrun'])
    rwlog(cmd5,logfile,conf_dict['General']['dryrun'])
    ...
    if conf_dict['General']['format1'] == 'txt':
        ...
        wlog('reform barcode files',logfile)
    ...
    rwlog(cmdsort,logfile,conf_dict['General']['dryrun'])
    ...
    wlog('combine annotation and barcode on reads',logfile)
    if int(conf_dict['General']['dryrun']) == 0:
    ...
    rwlog(cmd6,logfile,conf_dict['General']['dryrun'])
    
    ### generate expression and QC matrix based on combined file
    wlog('generate expression matrix and QC matrix',logfile)
